let UAs = ['Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.100 Safari/537.36',
...
module.exports = {
  current: -1,
  next: function() {
    if (this.current === N - 1) this.current = -1;
    return UAs[++this.current];
  },
  random: function() {
    const n = Math.floor(Math.random() * (N - 1)) + 0;
    return UAs[n];
  },
};
